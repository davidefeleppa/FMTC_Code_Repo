...
% Initialize variables to track global min/max
global_min = inf;
global_max = -inf;

% First pass: compute all deltas and find global min/max
all_deltas_a = cell(length(theta_values), 1);
all_deltas_b = cell(length(theta_values), 1);
...
for theta_idx = 1:length(theta_values)
    ...
    all_deltas_a{theta_idx} = deltas_a;
    all_deltas_b{theta_idx} = deltas_b;
    
    % Update global min/max
    global_min = min(global_min, min([deltas_a(:); deltas_b(:)]));
    global_max = max(global_max, max([deltas_a(:); deltas_b(:)]));
end
...
for theta_idx = 1:length(theta_values)
    theta = theta_values(theta_idx);
    deltas_a = all_deltas_a{theta_idx};
    deltas_b = all_deltas_b{theta_idx};
    ...
    title(sprintf('$\\theta=%.2f$', theta), 'Interpreter', 'latex', 'FontSize', 20);
    if theta_idx == length(theta_values)
        xlabel('Time', 'FontSize', 10);
    end
    ylabel('$\delta^{a, F \star}$', 'Interpreter', 'latex', 'FontSize', 20);
    xlim([0 T]);
    ...
    ylabel('$\delta^{b, F \star}$', 'Interpreter', 'latex', 'FontSize', 20);
    xlim([0 T]);
    ylim([y_min y_max]);  % Set consistent y-axis limits
end
