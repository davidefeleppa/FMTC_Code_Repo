%% Evaluate MM_Matrix over a Grid of (a, b) for 3 different theta values
% This script evaluates MM_Matrix over a grid of a(t) & b pairs for multiple theta values
% and plots the leader objective contour plots in 3 subplots on a single line.
% Beta is fixed at 0.05.
...
% Parameter Ranges
a_values = linspace(0, 1.0, 25);  % a values
b_values = linspace(0, 1.0, 25);  % b values (now varied instead of beta)
...
leader_results = zeros(num_a, num_b, num_theta);

% Loop through each theta, a, and b combination
fprintf('\n--- Running 3D Parameter Sweep for %d theta values ---\n', num_theta);
total_iterations = num_a * num_b * num_theta;
...
for k = 1:num_theta
    ...
    for i = 1:num_a
        for j = 1:num_b
            ...
            b_func = @(t) b_val;  % Use the current b value (now varied)
            
            % Run simulation with fixed beta
            ...
            % Progress Bar
            current_progress = (k-1)*num_a*num_b + (i-1)*num_b + j;
...
for k = 1:num_theta
    ...
    xlabel('$a$', 'Interpreter', 'latex', 'FontSize', 12, 'FontWeight', 'bold');
    ylabel('$b$', 'Interpreter', 'latex', 'FontSize', 12, 'FontWeight', 'bold');
    ...
    % hold on;
    % plot(a_values(opt_i), b_values(opt_j), 'r*', 'MarkerSize', 15, 'LineWidth', 3);
    % hold off;
    
    % Print optimal values
    fprintf('θ=%.3f: Optimal a=%.3f, b=%.3f, objective=%.3f\n', ...
        theta_values(k), a_values(opt_i), b_values(opt_j), max_val);
end

% Add overall title with LaTeX interpreter
% sgtitle('Leader Objective Function for Different $\theta$ Values ($\beta = 0.05$)', ...
%    'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold');

% Create summary table of optimal values
fprintf('\n=== SUMMARY OF OPTIMAL VALUES (β = %.3f) ===\n', beta_val);
fprintf('θ\t\tOptimal a\tOptimal b\tMax Objective\n');
fprintf('-----------------------------------------------\n');
for k = 1:num_theta
    [max_val, max_idx] = max(leader_results(:, :, k), [], 'all');
    [opt_i, opt_j] = ind2sub(size(leader_results(:, :, k)), max_idx);
